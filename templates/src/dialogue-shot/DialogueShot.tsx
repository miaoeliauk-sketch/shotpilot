 ...
 *
 * 这是工作台里用的版本：参数由 params.ts 从中文表单的简单选项换算出来。
 * 带逐帧实测记录、可在 Remotion Studio 里验证的原始版本在 examples/replica-s003。
 */

type Bezier = [number, number, number, number];
type Segment = { start: number; end: number; easing: Bezier };
...
export type Move = {
  ...
  /** 入场时的起始 / 离场时的最终不透明度 */
  opacity: number;
  /** 透明度变化的时间段（和位移分开，原片两者节奏不同） */
  fade: Segment;
  /** 入场起始模糊（像素）和模糊消失的帧；离场不用 */
  blur: number;
  blurEnd: number;
};
...
export type DialogueShotProps = {
  /** 底图地址：以 / 开头或带协议的直接用，否则当作 public/ 下的文件 */
  image: string;
  /** 可选：开场特写专用的高清底图，空字符串 = 不用 */
  zoomImage: string;
  camera: {
    startScale: number;
    ...
    unrotate: Segment;
    pushIn: Segment;
    zoomImageFade: [number, number];
  };
  motionBlur: { enabled: boolean; shutter: number; samples: number; minSpeed: number };
  bubbleStyle: {
    fontSize: number;
    fontWeight: number;
    textOffsetY: number;
...
const World: React.FC<{ t: number; props: DialogueShotProps }> = ({ t, props }) => {
  ...
  return (
    ...
    >
      <Img src={assetUrl(props.image)} style={{ position: 'absolute', left: 0, top: 0, width: 1280, height: 720 }} />
      {zoomOpacity > 0 && (
        <Img
          src={assetUrl(props.zoomImage)}
