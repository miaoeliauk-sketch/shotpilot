 ...
 *
 * 原因有二：正文要一个字一个字动，每个字得是单独的块；
 * 划线、荧光笔要画在重点句下面，得知道重点句落在哪几行、每行从哪到哪。
 * 规则：左对齐、首行缩进，逗号句号不放行首（挂在行尾），引号开头不放行尾，英文单词不拆开。
 */

/** 一段正文：文字、要标出来的重点句、整段左右挪多少 */
export type Paragraph = { text: string; highlight: string; offsetX?: number };
