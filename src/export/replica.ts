...
- 看报告里有没有「局部越往后越不像」（这时退出码是 3）。先看差异视频分清两种原因：
  有缓慢的动效没还原，还是那块区域后半段才出现元素（例如字体对不上的文字）把误差拉高了
...
5. **发虚不一定是变淡。** 快速移动的元素带运动模糊，模板匹配会把模糊算成透明度下降。
   先量边缘的模糊宽度 ÷ 每帧位移（得到快门），扣掉模糊后再判断是不是真的在淡出。
6. **被口播字幕挡住的那几帧要单独看。** 比对和追踪都会遮掉字幕区，从画面底部升上来的元素，
   前几帧正好藏在那里，测出来的入场起点会偏晚。逐帧肉眼确认它第一次出现的位置。
7. **从特写拉远的镜头，底图要够清楚。** 开场放大 3 倍时，拿正常画面放大当底图会糊。
   用特写那几帧反推一张高清底图，或者让用户提供高清原图。
8. **看起来是纯色的，逐列量一下。** 深色气泡的底色实测是中间亮两边暗的横向渐变，差 14 个灰度，
   肉眼看不出，像素误差能看出来。
